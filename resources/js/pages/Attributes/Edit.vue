...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/attributes/${props.attribute.id}/edit` },
];
...
const form = useForm({
    ...
    values: [...props.attribute.values] as any[],
});

const newValue = ref('');

// Initialize color values from existing data
const initialColorValues = props.attribute.values
    .filter(v => v.hex) // Only values with hex are color values
    .map(v => ({
        name: v.value,
        hex: v.hex || '#ffffff'
    }));

const colorValues = ref<ColorValue[]>(initialColorValues);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-foreground">Edit Attribute</h1>
                    <p class="text-sm text-muted-foreground">Modify the attribute details and values</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-end gap-4">
                    <Button 
                        type="button" 
                        variant="outline" 
                        @click="$inertia.visit(route('attributes.show', attribute.id))"
                    >
                        Cancel
                    </Button>
                    <Button 
