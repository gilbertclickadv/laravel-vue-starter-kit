...
const form = useForm({
    ...
    additional_data: props.item.additional_data || {},
...
const submit = () => {
    form.post(route('admin.homepage-sections.items.update', [props.section.id, props.item.id]), {
        onSuccess: () => {
            toast({
                title: 'Success',
...
<template>
    <AppLayout>
        <Head title="Edit Section Item" />

        <div class="container py-6">
            <Card>
                <CardHeader>
                    <CardTitle>Edit Item in {{ section.title }}</CardTitle>
                </CardHeader>
